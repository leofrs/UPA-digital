...
const MakeAnAppointment = () => {
  ...
  const validationSchema = z.object({
    medicos: z.string().nonempty("É obrigatório selecionar um médico"),
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!data.medicos || !data.data || !data.descricao) {
        setFormStatus({
          show: true,
          success: false,
      ...
      <div className="w-9/12 bg-white rounded-lg shadow-lg p-8">
        ...
        >
          <div className="space-y-6">
            <div>
              <label htmlFor="medicos" className="block text-sm font-medium text-gray-600 mb-2">
                Médico:
              </label>
              <select
                id="medicos"
                ...
                className={`w-full px-4 py-3 bg-gray-50 border-2 rounded-xl ${errors.medicos ? "border-red-400" : "border-gray-200"}`}
              >
              ...
              {errors.medicos && (
                <div className="text-red-500 text-sm mt-2">{errors.medicos.message}</div>
              )}
            </div>
